...
class TechAnalysis (object):
    ...
    def STO(df, nK, nD, nS=1):
        SOk = pd.Series(
            ...
            name='SO%k' + str(nK))
        SOd = pd.Series(SOk.ewm(ignore_na=False, span=nD, min_periods=nD - 1, adjust=True).mean(),
                        name='SO%d' + str(nD))
    ...
    def MACD(self, df, n_fast, n_slow):
        mac26 = df['Adj Close'].ewm(span=n_slow, adjust=False).mean()
        mac12 = df['Adj Close'].ewm(span=n_fast, adjust=False).mean()
        macd = mac12 - mac26
    ...
    def KST(df, r1, r2, r3, r4, n1, n2, n3, n4):
        ...
        ROC4 = M / N
        KST = pd.Series(
            pd.rolling_sum(ROC1, n1) + pd.rolling_sum(ROC2, n2) * 2 + pd.rolling_sum(ROC3, n3) * 3 + pd.rolling_sum(
                ROC4, n4) * 4,
    ...
    def KELCH(df, n):
        ...
        KelChU = pd.Series(pd.rolling_mean((4 * df['High'] - 2 * df['Low'] + df['Close']) / 3, n),
                           name='KelChU_' + str(n))
        KelChD = pd.Series(pd.rolling_mean((-2 * df['High'] + 4 * df['Low'] + df['Close']) / 3, n),
                           name='KelChD_' + str(n))
        df = df.join(KelChM)
        df = df.join(KelChU)
        df = df.join(KelChD)
